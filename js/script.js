...
$(document).ready(function(){
    ...
    $.getJSON("/data/JSONData.txt")
        .success(start)
        // Adding alert if server is not running
        .fail(function() {
            alert("You are probably not running this with python's simplehttpserver\n\
If you are using python3 or higher version do: python3 -m http.server\n\
If you are using python2.7 or lower version do: python -m SimpleHTTPServer\n\
After that you can access the page on: http://localhost:8000")
    });
    // Creating the report table and combo boxes
    function start(jsonData) {
        var $reportTableContainer = $("#table-container");
        // Extract only value without type from the jsonData for later usage
        var data = jsonData["report"].map(function(item) {
                Object.keys(item).forEach(function(key) {
                    if (item[key].type === "date") {
                        item[key] = item[key].value.split(".").reverse().join("/");
                    } else {
        ...
        var tableReadyData = createFirstTableData(data);
        var $table = constructTable("report-table", tableReadyData);
